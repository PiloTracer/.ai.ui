...
**2026-08-19 — ui-deploy-repo removed (operator-directed consistency):** deleted `skills/ui-deploy-repo/` + `scripts/ui-deploy-repo.sh`; every reference swept — `skills/README.md`, `SKILL_DEPENDENCIES.md` (matrix / redirects / vocabulary), `ui-director` skill + reference, `ui-deploy-files` skill.md + shell exclude regex, `framework-verify.sh` (existence check + smoke selftests), `cursorrules-verify.sh` header, `templates/cursorrules.ui.template`, `.quick/deploy-to-project.md`. Deploy family is now `ui-deploy-basic` + `ui-deploy-files` only — parity with Agent OS (`deploy-repo` removed 2026-08-14) and parent orchestrator `pilo.ai.logicbison` (basic + files). Historical CHANGELOG/HANDOFF entries left intact. Gates: touch-scope PASS, blast-radius PASS (approved in touch-scope), framework-verify PASS (18 skills derived), gate-verify PASS.
...
**2026-08-13 — ui-session parity + dual-scope batch:** `ui-session` brought to full `@session-control` parity — verbs `start` / `status` / `context` / `add` / `commit` / `close` / `push` (any combination), `scoped` modifier (bookends only), aliases, combination guards, secrets scan, ref-prefixed default commit message (`UIS-123: …` else `chore: …`). **Dual scope (owner-requested):** framework source repo → whole-tree add/commit/push; adopter target repos → strictly `.work.ui/` (scope guard + push history check). New `skills/ui-session/reference.md` (S1–S6 / X1–X3 / M1–M6 / C1–C8 protocols, verb×action matrix, edge cases). Reference sweep: `skills/README.md`, `SKILL_DEPENDENCIES.md`, `ui-director` (+reference), `ui-process-router` reference, `PROCESS_ROUTER.md`, `context/README.md`, `COHABITATION.md`, `START_HERE.md`, cursorrules + work.ui templates, `.work.ui/README.md`, CHANGELOG `[Unreleased]`. Self-test extended (16 behavioral cases incl. framework whole-tree mode). Gates: `touch-scope-verify` PASS, `blast-radius-check` PASS (approved in touch-scope), `framework-verify` PASS (incl. ui-session self-test + markdown link scan), `gate-verify` PASS.
...
|------|---------|-----------|
| 2026-08-19 | ui-deploy-repo removed (consistency) | deleted `skills/ui-deploy-repo/` + `scripts/ui-deploy-repo.sh`; swept registries (`skills/README.md`, `SKILL_DEPENDENCIES.md`, `ui-director`, `ui-deploy-files`), verifiers (`framework-verify.sh`, `cursorrules-verify.sh`, `ui-deploy-files.sh`), `templates/cursorrules.ui.template`, `.quick/deploy-to-project.md`; CHANGELOG [Unreleased]; gates PASS |
