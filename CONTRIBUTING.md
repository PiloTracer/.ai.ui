...
5. When adding a new `ui-*` skill, also update [`skills/ui-director/reference.md`](skills/ui-director/reference.md) §6 routing table and [`skills/ui-process-router/reference.md`](skills/ui-process-router/reference.md) bucket list.
6. Run `bash scripts/framework-verify.sh` before opening a PR (derives skill count, cross-checks registration, prose counts, intake contract, and self-tests `readiness-verify` + `traceability-verify`).
