...
**Deploy to:** UI Design OS (`.ai`, `.ai.ui`, `.ai.biz`, `.ai.soc`) — optional integration.
...
| Invocation | Mode |
|-----------|------|
| `@ui-project-query-setup install` | Full guided setup: generate key → create file → test → register MCP |
| `@ui-project-query-setup status` | Check if connected (key file exists? API reachable? MCP tools listed?) |
| `@ui-project-query-setup key` | Guide the user through web UI → key creation → `~/.tools-project-key` |
| `@ui-project-query-setup test` | Verify connectivity by listing projects |
| `@ui-project-query-setup register-mcp` | Register MCP server in consuming project's opencode.json |
| `@ui-project-query-setup help` | Show available tools and usage patterns for this OS |
| `@ui-project-query-setup` (no verb) | Default to `status` mode |
