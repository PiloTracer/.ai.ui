...
- **Deploy parity: `ui-deploy-basic` fills sister cells at bootstrap.** The substitution step now discovers installed sisters next to the source and pins `REPLACE:AI_<FW>_PATH` → `<abs> (discovered at deploy time)`; absent sisters keep their token with a checked-candidates report. `cursorrules-verify.sh` checks all six slots + `.ai` via the shared lib, resolves `*source via VAR*` cells, treats uninstalled frameworks as degraded (warn), and `--fix` pins unfilled cells (full cell shape) and repoints stale absolute cells. Self-tests added in `framework-verify.sh` (deploy-time pinning per slot, registry rows, `sister_names` legacy + family naming, framework-root self-audit).
- **`skills/ui-copy` stale reference fixed** — `@content-writing` (does not exist in `.ai.biz`) → `@biz-writing` / `@biz-content`.

- **`ui-session` full session-control parity + dual-scope model.** The `.work.ui`-scoped carrier now mirrors Agent OS `@session-control` verbs end-to-end: **`start` [- goal]** (marks the UI session open in `HANDOFF_UI.md` — `Open: <date> - goal: …`, resets `Closed`), **`context`** (read-only, uncommitted-aware snapshot: staged/unstaged/untracked counts + secrets flags, paths only), **`add`** (stage-only checkpoint, incl. untracked files/dirs, never commits), **`scoped`** modifier (bookend files `HANDOFF_UI.md` + `NEXT_UI.md` only; other staged paths are unstaged first, kept as working-tree changes), aliases (`begin`/`open`→start, `end`/`handoff`→close, `stage`→add), and combination guards (`start close` and read-only+write mixes are refused). **Dual scope, auto-detected:** in the framework source repo (repo root carries `COHABITATION.md` + `skills/ui-session/skill.md` + `templates/bootstrap.sh`) `add`/`commit`/`push` cover the **whole repo tree**; in adopter target repos they remain strictly `.work.ui/`-scoped (scope guard + push history check unchanged). Every action line reports the active mode.
- **`ui-session` secrets scan + ref-prefixed default message.** Staged paths matching `credentials/`, `.env*`, `*.pem|p12|key|pfx|p8|token|secret`, `id_rsa` are never committed (script unstages + refuses, both modes). Default commit subject is now `<REF>: update … session state` when a ref is detectable (HANDOFF_UI Open goal → branch → last commit subject), else `chore: …` — aligned with the framework's `.cursorrules` git convention (`UIS-123: …` else `type: …`).
- **`skills/ui-session/reference.md`** — detailed protocols mirroring `session-control/reference.md`: start/status/context/commit/close protocols (S1–S6, X1–X3, M1–M6, C1–C8), verb×action matrix, HANDOFF_UI templates, commit-message rules + examples, critical interactions, anti-patterns, wrong-prompts table.
- **Self-test extended** (`scripts/ui-session.sh --self-test`, wired into `framework-verify.sh`): start open-line + goal, add stage-only, scoped bookend-only commit, secrets refusal (both modes), read-only context, contradictory/read-only+write combination refusal, ref default message + `chore:` fallback, mode reporting, framework-mode whole-tree commit / status / push of mixed history.
- **`scripts/cursorrules-verify.sh` — shared `.cursorrules` wiring auditor for all deploy skills.** Verifies a deployed target end-to-end: `.cursorrules` presence + UI rules, client mode (thin `AI_UI_SOURCE` / fat vendored `.ai.ui/` / framework-root), framework assets resolvable (`skills/README.md`), Source-resolution section present, sister framework paths (Frameworks registry `REPLACE:AGENT_OS_PATH` + `REPLACE:AI_<FW>_PATH` for the six slots) pinned or auto-discoverable from disk (family naming `pilo.ai.<fw>.logicbison` then legacy `.ai.<fw>` next to the framework), and `.work.ui/` presence. Strict exit codes (1 on blocking gap), `--report` (always 0), and `--fix` (pins sister paths from on-disk discovery; appends the Source-resolution section extracted from the mode-appropriate template). Wired as `verify [--fix]` / `status` into `ui-deploy-basic`, `ui-deploy-files`, and `ui-deploy-repo`, and runs as a report at the end of every deploy so each one closes with evidence of the target's wiring state.
...
- **Documented: child frameworks do not resolve the parent orchestrator (owner decision 2026-08-19).** `docs/homogenization/ui.md` records it: the `.ai` (Agent OS) registry cell stays legacy-only `../.ai`; orchestration is top-down — the parent (`x-director` in `pilo.ai.logicbison`) discovers children, children never look up; a missing parent is a degraded-warn, never a failure.
- **Reference sweep for the new verb set + dual scope** — `skills/README.md`, `skills/SKILL_DEPENDENCIES.md` (gate matrix, redirect cheat sheet, command vocabulary), `skills/ui-director/` (skill + reference routing rows), `skills/ui-process-router/reference.md`, `PROCESS_ROUTER.md`, `context/README.md`, `COHABITATION.md`, `START_HERE.md`, `templates/cursorrules.ui.template`, `templates/cursorrules.ui.snippet.template`, `templates/work.ui/README.md.template`, `templates/work.ui/context/HANDOFF_UI.md.template`, `.work.ui/README.md`. Historical dated records (CHANGELOG, HANDOFF_UI, reports) left intact.
- **`ui-session` default commit message** — was the untyped `ui-session: .work.ui update`; now ref-prefixed or `chore:`-typed per `.cursorrules` §Git (feeds the `post-commit` ref hook).
